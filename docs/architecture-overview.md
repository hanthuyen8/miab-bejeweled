# Kiến trúc & kỹ thuật của game Seajeweled
...
```cpp
GoSDL::Window(800, 600, "Seajeweled")   // Game.cpp — logical resolution cố định
...
- Bản Web (Emscripten): SDL2 qua "port" built-in, asset `media/` đóng gói
  vào `seajeweled.data` qua `--preload-file`. `options.json` nằm trong MEMFS
